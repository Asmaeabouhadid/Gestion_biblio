...
class LoginPage:
    def __init__(self):
        ...
        # Password label and entry
         self.password_entry = Entry(self.frame, width=25, fg='#AD956B',show="*", border=0, bg='white',  font=('Microsoft Yahei UI Light', 11))
        ...
        # Login button
         self.login_button = Button(self.frame, width=15, pady=8, text='Login', bg='#AD956B', fg='white', border=0, command=self.check_credentials)
    ...
    def check_credentials(self):
        ...
        query = "SELECT * FROM admin WHERE name = %s AND passeword	 = %s"
        ...
        if result:
            ...
            self.root.destroy()
            from homeA import Menu
            Menu() 
        else:
            # accès refusé - affichage d'un message d'erreur
            messagebox.showerror("Erreur", "Username or password incorrect.")
